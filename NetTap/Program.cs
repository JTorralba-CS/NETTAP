...
namespace NetTap
{
    class Program
    {
                ...
                if (Packet_Size > 0)
                {
                    ...
                    if (Settings_Data.AppSettings.Settings["ACK"].Value == "Auto" && Packet_String.Contains(HB_Request))
                    {
                        try
                        {
                            ...
                            Log(File + "Status", "Program" + " ---> " + Source_Remote_IPEndPoint.Address + ":" + String.Format("{0:000000}", Source_Remote_IPEndPoint.Port) + " " + String.Format("{0:000000}", HB_Acknowledgement_Bytes.Length) + " Byte(s)", HB_Acknowledgement);
                        }
                        catch (Exception E)
                        {
        ...
        static void Main(String[] Arguments)
        {
            ...
            try
            {
                Log("Status", "Main: Program().Start()", null);

                foreach (String IP in IP4_List())
                {
                    new Program().Start(IP, Listen_Port, new IPEndPoint(IPAddress.Parse(Destination_IP), Destination_Port));
                }
            }
            catch (Exception E)
            {
                Log("Exception", "Main: Program().Start()", E.Message);
            }
        }
    }
}
